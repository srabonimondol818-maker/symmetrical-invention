...
# Track ALL bot messages per user
user_all_messages = {}  # Format: {user_id: [message_ids]}

# 🔒 Permanent messages — কখনো delete হবে না (welcome, after_video)
user_permanent_messages = {}  # Format: {user_id: [message_ids]}

# 🎬 Current video message per user (নতুন video আসলে delete হবে)
user_current_video = {}  # Format: {user_id: message_id}
...
DEFAULT_SETTINGS = {
    ...
    'forward_protection': False,        # 🔒 Forward Protection: ON/OFF
}
...
async def track_message(user_id: int, message_id: int):
    """Temporary message track — পরে delete হবে"""
    if user_id not in user_all_messages:
        user_all_messages[user_id] = []
    user_all_messages[user_id].append(message_id)

async def track_permanent(user_id: int, message_id: int):
    """
    Permanent message track — কখনো delete হবে না।
    Welcome message ও after_video message এখানে।
    শুধু শেষ ২টা রাখো যাতে chat এ সবসময় কিছু থাকে।
    """
    if user_id not in user_permanent_messages:
        user_permanent_messages[user_id] = []
    user_permanent_messages[user_id].append(message_id)
    # শেষ ২টা permanent রাখো
    user_permanent_messages[user_id] = user_permanent_messages[user_id][-2:]
    # all_messages এও track করো
    if user_id not in user_all_messages:
        user_all_messages[user_id] = []
    if message_id not in user_all_messages[user_id]:
        user_all_messages[user_id].append(message_id)

async def track_video_msg(user_id: int, message_id: int):
    """Video message track — নতুন video আসলে delete হবে"""
    user_current_video[user_id] = message_id
    if user_id not in user_all_messages:
        user_all_messages[user_id] = []
    user_all_messages[user_id].append(message_id)

async def delete_old_video(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int):
    """নতুন video আসার আগে পুরনো video delete করো"""
    old_video_id = user_current_video.get(user_id)
    if old_video_id:
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=old_video_id)
            logger.info(f"🎬 Deleted old video {old_video_id} for user {user_id}")
        except Exception:
            pass
        user_current_video.pop(user_id, None)
        if user_id in user_all_messages:
            user_all_messages[user_id] = [m for m in user_all_messages[user_id] if m != old_video_id]

async def cleanup_temp_messages(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int):
    """
    🧹 Temporary messages delete করো (force_join, info messages)।
    Permanent (welcome, after_video) এবং video রাখো।
    Safety: সবসময় কমপক্ষে ১টা message থাকবে।
    """
    if not get_setting('message_cleanup_enabled', True):
        return

    permanent = set(user_permanent_messages.get(user_id, []))
    current_video = user_current_video.get(user_id)
    protected = permanent | ({current_video} if current_video else set())

    all_msgs = user_all_messages.get(user_id, [])
    to_delete = [m for m in all_msgs if m not in protected]

    # Safety: যদি delete করলে chat empty হয়ে যাবে, শেষেরটা রেখে দাও
    if len(to_delete) == len(all_msgs) and len(to_delete) > 0:
        to_delete = to_delete[:-1]  # শেষেরটা রাখো
    ...
    user_all_messages[user_id] = [m for m in all_msgs if m not in set(to_delete)]
    if to_delete:
        logger.info(f"🧹 Cleaned {len(to_delete)} temp messages for user {user_id}")

async def ensure_not_empty(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int):
    """
    Safety check: যদি কোনো কারণে ৩টা+ message জমে যায়,
    শেষেরটা রেখে বাকি সব delete করো।
    """
    all_msgs = user_all_messages.get(user_id, [])
    if len(all_msgs) >= 3:
        permanent = set(user_permanent_messages.get(user_id, []))
        ...
        if to_delete:
            for msg_id in to_delete:
                try:
                    ...
                    pass
            user_all_messages[user_id] = [m for m in all_msgs if m not in set(to_delete)]
            logger.info(f"🛡️ Safety cleanup: removed {len(to_delete)} messages for user {user_id}")

# Backward compatibility
async def cleanup_user_messages(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int, keep_last: int = 0):
    await cleanup_temp_messages(context, user_id, chat_id)
...
async def edit_or_send(
    ...
    try:
        if is_query:
            await query_or_context.message.edit_text(text, reply_markup=reply_markup, parse_mode=parse_mode)
        else:
            context = query_or_context
            return await context.bot.send_message(
                chat_id=context._chat_id, text=text, reply_markup=reply_markup, parse_mode=parse_mode
            )
    except Exception:
        if is_query:
            return await query_or_context.message.reply_text(text, reply_markup=reply_markup, parse_mode=parse_mode)
...
async def refresh_settings_panel(query):
    """Refresh the settings panel after a toggle"""
    ...
    forward_protection = get_setting('forward_protection', False)
...
🔒 **Video Protection:** {"🔒 ON" if protection else "🔓 OFF"}
💬 **Auto Reply:** {"✅ ON" if auto_reply else "❌ OFF"}
🧹 **Message Cleanup:** {"✅ ON" if cleanup else "❌ OFF"}
🎬 **Welcome Media:** {"✅ ON" if welcome_media else "❌ OFF"}
🎙️ **Welcome Voice:** {"✅ ON" if welcome_voice else "❌ OFF"}
📁 **Folder Join Link:** {"✅ ON" if folder_enabled else "❌ OFF"}
❌ **Video Not Found Button:** {"✅ ON" if notfound_btn else "❌ OFF"}
🚫 **Forward Protection:** {"✅ ON" if forward_protection else "❌ OFF"}
...
def settings_keyboard():
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🚫 Forward Protection", callback_data="setting_forward_protection")],
        [InlineKeyboardButton("🔙 Back", callback_data="admin_main")]
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def send_direct_video_by_code(user_id, code, context):
    ...
    try:
        ...
        # 🧹 পুরনো video delete, temp clean
        await delete_old_video(context, user_id, user_id)
        await cleanup_temp_messages(context, user_id, user_id)
        ...
        fp = get_setting('forward_protection', False)
        ...
        await track_video_msg(user_id, video_msg.message_id)
        await track_permanent(user_id, after_msg.message_id)
        await ensure_not_empty(context, user_id, user_id)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and len(context.args) > 0:
        ...
        # Deeplink এ শুধু /start command delete, আর কিছু না
        try:
    ...
    if custom_buttons:
        ...
        # Voice on/off check করে keyboard ঠিক করো
        welcome_voice_enabled = get_setting('welcome_voice_enabled', False)
        
        if welcome_voice_enabled:
            # Voice ON → App button শুধু welcome text এ, Join+Help voice এর নিচে
            keyboard = [[app_button]]
        else:
            # Voice OFF → সব button welcome text এর নিচে একসাথে
            if main_channel.startswith("https://"):
                channel_url = main_channel
            else:
                channel_url = f"https://t.me/{main_channel}"
            keyboard = [
                [app_button],
    ...
    try:
        # 🧹 Welcome screen এ temp messages clean করো
        await cleanup_temp_messages(context, user.id, update.effective_chat.id)
        ...
        if welcome_media_enabled:
            ...
            if media_file_id and media_type:
                try:
                    ...
                    fp = get_setting('forward_protection', False)
                    if media_type == 'photo':
                        media_msg = await update.message.reply_photo(photo=media_file_id, protect_content=fp)
                    elif media_type == 'animation':
                        media_msg = await update.message.reply_animation(animation=media_file_id, protect_content=fp)
                    elif media_type == 'video':
                        media_msg = await update.message.reply_video(video=media_file_id, protect_content=fp)
        ...
        # ✅ Welcome message permanent — কখনো delete হবে না
        await track_permanent(user.id, sent_msg.message_id)
        
        # ③ 🎙️ Voice ON হলে — voice + Join/Help button নিচে
...
async def handle_direct_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    try:
        ...
        # 🧹 পুরনো video delete, temp clean
        await delete_old_video(context, user.id, chat_id)
        await cleanup_temp_messages(context, user.id, chat_id)
        ...
        await track_video_msg(user.id, video_msg.message_id)
        await track_permanent(user.id, after_msg.message_id)
        await ensure_not_empty(context, user.id, chat_id)
...
Select an option below:"""
    ...
    # 🔥 NEW: Verified Video Channels
        ...
        forward_protection_status = "✅ ON" if forward_protection else "❌ OFF"
...
🚫 **Forward Protection:** {forward_protection_status}
...
Select an option below:"""
    ...
    # 🔥 NEW: Remove Verified Channel
        ...
        if setting_key == "protection":
            ...
            await query.answer(f"Video Protection: {status}", show_alert=True)
            await refresh_settings_panel(query)
            return
            ...
            status = "✅ ON" if new_val else "❌ OFF"
            await query.answer(f"Auto Reply: {status}", show_alert=True)
            await refresh_settings_panel(query)
            return
            ...
            status = "✅ ON" if new_val else "❌ OFF"
            await query.answer(f"Message Cleanup: {status}", show_alert=True)
            await refresh_settings_panel(query)
            return
            ...
            status = "✅ ON" if new_val else "❌ OFF"
            await query.answer(f"Video Not Found Button: {status}", show_alert=True)
            await refresh_settings_panel(query)
            return
        ...
        # NEW: Welcome Media management
            ...
            if not current:
                ...
                await query.answer("✅ Welcome Media: OFF", show_alert=True)
                await refresh_settings_panel(query)
            return
        
        # 🎙️ NEW: Welcome Voice management
        elif setting_key == "forward_protection":
            current = get_setting('forward_protection', False)
            new_val = not current
            set_setting('forward_protection', new_val)
            status = "✅ ON" if new_val else "❌ OFF"
            await query.answer(f"🚫 Forward Protection: {status}", show_alert=True)
            await refresh_settings_panel(query)
            return
            ...
            if not current:
                ...
                await query.answer("🎙️ Welcome Voice: OFF", show_alert=True)
                await refresh_settings_panel(query)
            return
        ...
        # NEW: Folder Link management
            ...
            if not current_enabled or not current_url:
                ...
                await query.answer("✅ Folder Link: OFF", show_alert=True)
                await refresh_settings_panel(query)
            return
        ...
        hints = {
            ...
            'main_channel': "Public: YourChannel (without @)\nPrivate: https://t.me/+xxxxxxxxxx",
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔥 NEW: Add Verified Channel Handler
        ...
        if setting_key == 'main_channel_id':
            ...
            # Private invite link (https://t.me/+xxx) হলে as-is রাখো
            # Public username হলে @ সরিয়ে দাও
