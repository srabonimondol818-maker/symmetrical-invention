...
def build_force_join_keyboard(all_channels, not_joined_ids, video_id, is_direct=False):
    ...
    Folder link enabled থাকলে single button দেখাবে।
    """
    keyboard = []

    # Folder link enabled? → single button দেখাও
    ...
    for i, ch in enumerate(all_channels):
        ...
        if ch_id not in not_joined_ids:
            # Joined — ✅ দেখাবে, click করা যাবে না
            keyboard.append([InlineKeyboardButton(f"✅  {name}", callback_data="noop_joined")])
        else:
...
async def check_channel_access(bot, user_id, channel):
    ...
    try:
        ...
        if member.status in ['member', 'administrator', 'creator', 'restricted']:
            ...
            # Channel থেকে বের হয়ে গেছে
            # Private channel: bot এইমাত্র approve করেছে কিন্তু Telegram reflect করেনি?
            # শুধু সেই ক্ষেত্রে DB check করো
            if channel.get('invite_link'):  # private channel
                if has_pending_join_request(user_id, channel_id):
                    logger.debug(f"⏳ Private channel pending: user {user_id} in {channel_id}")
                    return True
            # DB record মুছে দাও (user বের হয়ে গেছে)
            try:
                pending_requests_col.delete_one({'user_id': user_id, 'channel_id': channel_id})
            except:
                pass
