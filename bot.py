...
# Track current video message per user — শুধু 1টা video থাকবে
...
async def track_video_msg(user_id: int, message_id: int):
    """Video message track — পুরনোটা replace হবে, শুধু 1টা video থাকবে"""
...
async def delete_old_video(context, user_id: int, chat_id: int):
    """পুরনো video message delete করো"""
    old_id = user_current_video.get(user_id)
    if old_id:
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=old_id)
            logger.info(f"🎬 Deleted old video {old_id} for user {user_id}")
        except Exception:
            pass
        user_current_video.pop(user_id, None)
        if user_id in user_all_messages:
            user_all_messages[user_id] = [m for m in user_all_messages[user_id] if m != old_id]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🔍 save_user এর আগে নতুন user কিনা check করো
    ...
    if context.args and len(context.args) > 0:
        ...
        # নতুন user → /start message delete করো না (Telegram intro card থাকবে)
        # পুরনো user → /start message delete করো (clean রাখতে)
        if not is_new_user:
            try:
                await context.bot.delete_message(
        ...
        if video_id.isdigit():
            await handle_video_request(update, context, video_id, is_new_user=is_new_user)
        else:
            await handle_direct_video_request(update, context, video_id, is_new_user=is_new_user)
        return
...
async def handle_direct_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str, is_new_user: bool = False):
    """Handle direct upload video request by unique code"""
    user = update.effective_user
    chat_id = update.effective_chat.id

    # নতুন user → কিছু clean করো না (সব message থাকবে)
    # পুরনো user → আগের video delete করো + last 1টা message রেখে বাকি clean করো
    if not is_new_user:
        await delete_old_video(context, user.id, chat_id)
        await cleanup_user_messages(context, user.id, chat_id, keep_last=1)
...
async def handle_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str, is_new_user: bool = False):
    """Handle video playback request"""
    user = update.effective_user
    chat_id = update.effective_chat.id

    # নতুন user → কিছু clean করো না
    # পুরনো user → আগের video delete + last 1টা message রেখে বাকি clean
    if not is_new_user:
        await delete_old_video(context, user.id, chat_id)
        await cleanup_user_messages(context, user.id, chat_id, keep_last=1)
