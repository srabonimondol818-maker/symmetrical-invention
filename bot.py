...
# 🔥 Track video messages separately — these should NOT be deleted on cleanup
user_video_protected = {}  # Format: {user_id: [message_ids]} — video + after_video messages
...
async def cleanup_ui_messages(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int):
    """
    🧹 শুধু UI messages delete করে (welcome, force_join, info)
    Video messages কখনো delete করে না।
    """
    cleanup_enabled = get_setting('message_cleanup_enabled', True)
    if not cleanup_enabled:
        return

    protected_ids = set(user_video_protected.get(user_id, []))
    all_msgs = user_all_messages.get(user_id, [])
    to_delete = [m for m in all_msgs if m not in protected_ids]
    ...
    if deleted > 0:
        logger.info(f"🧹 Cleaned {deleted} UI messages for user {user_id}")

    # Keep only video messages in tracking list
    user_all_messages[user_id] = [m for m in all_msgs if m in protected_ids]


async def cleanup_old_video(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int):
    """
    🎬 নতুন video আসার আগে পুরনো video + after_video delete করে।
    Chat এ সবসময় শুধু একটাই video থাকবে।
    """
    old_video_ids = user_video_protected.get(user_id, [])
    deleted = 0
    for msg_id in old_video_ids:
        try:
            await context.bot.delete_message(chat_id=chat_id, message_id=msg_id)
            deleted += 1
        except Exception:
    ...
    if deleted > 0:
        logger.info(f"🎬 Cleared {deleted} old video messages for user {user_id}")

    # Reset protected list
    user_video_protected[user_id] = []
    # Remove from tracking too
    user_all_messages[user_id] = [m for m in user_all_messages.get(user_id, [])
                                   if m not in set(old_video_ids)]


# Keep old name for compatibility
async def cleanup_user_messages(context: ContextTypes.DEFAULT_TYPE, user_id: int, chat_id: int, keep_last: int = 0):
    await cleanup_ui_messages(context, user_id, chat_id)


async def track_message(user_id: int, message_id: int):
    """UI message track করো — cleanup এ delete হবে"""
    if user_id not in user_all_messages:
        user_all_messages[user_id] = []
    user_all_messages[user_id].append(message_id)


async def track_video_message(user_id: int, message_id: int):
    """Video message track করো — সাধারণ cleanup এ delete হবে না"""
    if user_id not in user_all_messages:
        user_all_messages[user_id] = []
    user_all_messages[user_id].append(message_id)
    if user_id not in user_video_protected:
        user_video_protected[user_id] = []
    user_video_protected[user_id].append(message_id)
...
async def send_direct_video_by_code(user_id, code, context):
    ...
    try:
        ...
        # 🧹 UI messages clean করো, পুরনো video delete করো
        await cleanup_ui_messages(context, user_id, user_id)
        await cleanup_old_video(context, user_id, user_id)
        ...
        await track_video_message(user_id, video_msg.message_id)
        await track_video_message(user_id, after_msg.message_id)
...
async def send_video_direct_by_id(user_id, video_id, context):
    ...
    try:
        ...
        # Track for future cleanup — protected
        await track_video_message(user_id, video_msg.message_id)
        await track_video_message(user_id, after_msg.message_id)
...
async def handle_direct_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    try:
        ...
        # 🎬 পুরনো video delete করো, নতুন video আসার আগে
        await cleanup_old_video(context, user.id, chat_id)
        ...
        await track_video_message(user.id, video_msg.message_id)
        await track_video_message(user.id, after_msg.message_id)
...
async def handle_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str):
    ...
    try:
        ...
        # Track these messages — protected from cleanup
        await track_video_message(user.id, video_msg.message_id)
        await track_video_message(user.id, after_msg.message_id)
