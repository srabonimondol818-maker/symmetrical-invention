...
def get_video_not_found_keyboard():
    ...
    if mini_app_url.startswith('https://t.me/'):
        return InlineKeyboardMarkup([[InlineKeyboardButton("🎬 Open CINEFLIX App", url=mini_app_url)]])
...
def make_mini_app_button(text="🎮 Open CINEFLIX App"):
    ...
    t.me link → url button (Mini App as direct link)
    vercel/other → web_app button (opens inside Telegram)
    """
    mini_app_url = get_setting('mini_app_url', DEFAULT_SETTINGS['mini_app_url'])
    if mini_app_url.startswith('https://t.me/'):
        return InlineKeyboardButton(text, url=mini_app_url)
    return InlineKeyboardButton(text, web_app={"url": mini_app_url})
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if custom_buttons:
        ...
        # t.me link হলে url button, নয়তো web_app button
        if mini_app_url.startswith('https://t.me/'):
...
Select an option below:"""
    ...
    # ===================== 📢 POST MANAGER CALLBACKS =====================
        ...
        text = (
            ...
            f"✅ Channel select করে bot থেকেই post করুন\n"
            f"✅ Channel এ post হলে এখানে edit করুন\n"
            f"✅ Button যোগ করুন, caption edit করুন\n\n"
            f"👇 Channel বেছে নিন:"
        )
