...
# Current video per user — শুধু ১টা video থাকবে
...
async def track_video_msg(user_id: int, message_id: int):
    """Video message track — শুধু ১টা video থাকবে"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # save_user এর আগে check — নতুন user কিনা
    ...
    # ── Case A: Deeplink দিয়ে video request ──
    if context.args and len(context.args) > 0:
        video_id = context.args[0]

        # Deeplink case এ /start message কখনোই delete করো না
        # নতুন বা পুরনো যেই হোক — /start থাকবে
        # পুরনো user হলে শুধু আগের bot messages clean করো
        if not is_new_user:
    ...
    # ── Case B: Plain /start (কোনো deeplink নেই) ──
    if not is_new_user:
        # পুরনো user → আগের welcome clean করো, /start delete করো
        await cleanup_user_messages(context, user.id, update.effective_chat.id)
        try:
            await context.bot.delete_message(
            ...
            pass
    # নতুন user → কিছু করো না, শুধু welcome দাও
...
async def handle_direct_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    # পুরনো video delete করো (cleanup start এ হয়েছে)
...
async def admin_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_states.get(user_id, {}).get('action') == 'voice_upload':
        # Allow /cancel to fall through to the cancel handler below
        if update.message and update.message.text and update.message.text == '/cancel':
            pass  # fall through to cancel handler
        elif update.message and (update.message.audio or update.message.voice or
    ...
    if text is None and state.get('action') in text_only_states:
        # Check if caption has /cancel
        caption = update.message.caption if update.message else None
        if caption and caption.strip() == '/cancel':
            del admin_states[user_id]
            await update.message.reply_text("❌ Cancelled")
            return
        await update.message.reply_text(
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /cancel command from any message type"""
...
def main():
    ...
    application.add_handler(MessageHandler(
        filters.ChatType.PRIVATE & filters.TEXT & (~filters.COMMAND | filters.Regex(r'^/cancel$')),
