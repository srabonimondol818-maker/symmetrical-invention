...
def build_force_join_keyboard(all_channels, not_joined_ids, video_id, is_direct=False):
    ...
    ✅ = joined/pending, 🔒 = not joined (clickable)
    নিচে Verify 🔐 button সবসময় থাকবে।
    সব joined হলে Verify তে click করলে video যাবে।
    """
    keyboard = []
    ...
    for i, ch in enumerate(all_channels):
        ...
        if ch_id not in not_joined_ids:
            # Joined বা pending — ✅ দেখাবে, click করা যাবে না
            keyboard.append([InlineKeyboardButton(f"✅  {name}", callback_data="noop_joined")])
        else:
            # Not joined — 🔒 দেখাবে, click করলে join page যাবে
            if invite_link:
                url = invite_link
            elif username:
                url = f"https://t.me/{username}"
            else:
                keyboard.append([InlineKeyboardButton(f"🔒  {name}", callback_data="noop_locked")])
                continue
            keyboard.append([InlineKeyboardButton(f"🔒  {name}", url=url)])

    # Verify button
    cb = f"verify_direct_{video_id}" if is_direct else f"verify_{video_id}"
    keyboard.append([InlineKeyboardButton("· · · · · · · · · · · · · ·", callback_data="noop_sep")])
    keyboard.append([InlineKeyboardButton("🔐  Verify & Unlock", callback_data=cb)])
...
def build_force_join_text(all_channels, not_joined_ids):
    ...
    if done == 0:
        return (
            f"🔒 **Content Locked**\n\n"
            f"ভিডিওটি unlock করতে নিচের **{total}টি channel** join করুন।\n\n"
            f"📊 Progress: {progress} `0/{total}`\n\n"
            f"Channel গুলোতে join করুন, তারপর **Verify 🔐** চাপুন।"
        )
    elif remaining == 1:
        return (
            f"🔒 **Content Locked**\n\n"
            f"📊 Progress: {progress} `{done}/{total}`\n\n"
            f"✅ {done}টি হয়েছে! মাত্র **১টি** বাকি।\n"
            f"শেষ channel join করে **Verify 🔐** চাপুন।"
        )
    else:
        return (
            f"🔒 **Content Locked**\n\n"
            f"📊 Progress: {progress} `{done}/{total}`\n\n"
            f"✅ {done}টি হয়েছে — আরও **{remaining}টি** বাকি।\n"
            f"বাকি channel গুলো join করে **Verify 🔐** চাপুন।"
        )
...
async def check_channel_access(bot, user_id, channel):
    ...
    Access = member ✅ OR bot-approved ✅ OR pending request ✅
    
    Logic:
    1. DB তে approved/pending record আছে → access দাও (bot already approved)
    2. Telegram API তে member/admin/creator → access দাও
    3. left/kicked AND no DB record → no access
    4. Bot not admin in channel → skip (don't block user)
    """
    channel_id = channel['channel_id']

    # ── Step 1: DB check (fast, no API call) ──
    # Bot auto-approve করেছে বা pending আছে — দুটোতেই access
    if has_pending_join_request(user_id, channel_id):
        logger.debug(f"✅ DB access: user {user_id} in channel {channel_id}")
        return True

    # ── Step 2: Telegram API check ──
    try:
        member = await bot.get_chat_member(channel_id, user_id)

        if member.status in ['member', 'administrator', 'creator']:
            # Full member — mark approved in DB (future DB checks fast হবে)
            mark_join_approved(user_id, channel_id)
            return True

        elif member.status == 'restricted':
            # Restricted কিন্তু still member
            return True

        elif member.status in ['left', 'kicked']:
            # Channel এ নেই, DB তেও নেই → no access
            return False
        ...
        if 'chat not found' in err or 'bot is not a member' in err:
            # Bot এই channel এ admin না — এই channel skip করো, user block করো না
...
async def handle_chat_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if "user_already_participant" in error_msg or "hide_requester_missing" in error_msg:
            # User already in channel — just mark approved
            mark_join_approved(user.id, chat.id)
            logger.info(f"✅ Already member, marked approved: User {user.id}")
        elif "not enough rights" in error_msg:
            # Bot এর permission নেই — fallback: pending হিসেবে রাখো
            mark_join_request_sent(user.id, chat.id)
            logger.warning(f"⚠️ Bot cannot approve in channel {chat.id} — marked pending instead. Give bot 'Add Members' permission!")
        else:
            # অন্য error — pending mark করো
            mark_join_request_sent(user.id, chat.id)
...
async def handle_direct_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    status_map = await get_channel_status_map(context.bot, user.id)
    not_joined_ids = [cid for cid, s in status_map.items() if s == 'not_joined']
...
async def handle_direct_video_request_callback(query, context, code: str):
    ...
    try:
        ...
        if media_type == 'video':
            video_msg = await context.bot.send_video(chat_id=chat_id, video=file_id, protect_content=protect)
        elif media_type == 'document':
            video_msg = await context.bot.send_document(chat_id=chat_id, document=file_id, protect_content=protect)
        elif media_type == 'animation':
            video_msg = await context.bot.send_animation(chat_id=chat_id, animation=file_id, protect_content=protect)
        else:
            video_msg = await context.bot.send_video(chat_id=chat_id, video=file_id, protect_content=protect)
