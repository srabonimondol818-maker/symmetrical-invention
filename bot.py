...
def get_video_not_found_keyboard():
    """
    Video not found এ Mini App button দেখাবে — admin toggle করতে পারবে।
    Returns keyboard or None
    """
    show_button = get_setting('video_not_found_button', True)
    if not show_button:
        return None
    mini_app_url = get_setting('mini_app_url', DEFAULT_SETTINGS['mini_app_url'])
    return InlineKeyboardMarkup([[InlineKeyboardButton("🎬 Open CINEFLIX App", web_app={"url": mini_app_url})]])
...
async def handle_video_request(update: Update, context: ContextTypes.DEFAULT_TYPE, video_id: str):
    ...
    if not video:
        mini_app_url = get_setting('mini_app_url', DEFAULT_SETTINGS['mini_app_url'])
        keyboard = [[InlineKeyboardButton("🎬 Open CINEFLIX App", web_app={"url": mini_app_url})]]
        msg = await update.message.reply_text(
            get_message('video_not_found'),
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode=ParseMode.MARKDOWN
...
Select an option below:"""
    ...
    # ===================== 📢 POST MANAGER CALLBACKS =====================
        ...
        text = (
            "📢 **Post Manager**\n\n"
            "এই channel গুলো থেকে post হলে Admin কে forward করবে।\n"
            "Admin button যোগ করতে, caption edit করতে, বা delete করতে পারবে।\n\n"
            "👇 Channel যোগ করুন:"
        )
        await query.edit_message_text(text, reply_markup=post_manager_channel_keyboard(), parse_mode=ParseMode.MARKDOWN)
          ...
          data.startswith("pm_remove_") or data == "pm_add_channel"):
        if user_id != ADMIN_ID:
            return
    ...
    # ===================== 📌 PINNED BUTTON CALLBACKS =====================
        ...
        text = (
            ...
            "Channel এর pinned message এ buttons লাগাতে পারবেন।\n"
            "যতটা খুশি button দিতে পারবেন — url বা mini app।\n"
            "সব user সবসময় pinned message এ দেখতে পাবে ✅"
        )
...
async def channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not is_channel_verified(channel_id):
            # 📢 Check if it's a Post Manager channel (separate system)
            if is_post_manager_channel(channel_id):
                logger.info(f"📢 Post Manager channel detected: {channel_name} ({channel_id})")
                await handle_post_manager_channel_post(message, context)
            else:
        ...
        try:
            ...
            try:
                ...
                if bot_member.status not in ['administrator', 'creator']:
                    ...
                    return
                save_pinned_buttons(channel_id, [])
                await update.message.reply_text(
...
def get_post_manager_channels():
    try:
        return list(post_manager_channels_col.find({'is_active': True}))
    except Exception as e:
...
def build_post_buttons_keyboard(buttons, code):
    """Build keyboard from saved buttons list for preview"""
    keyboard = []
    for btn in buttons:
        if btn['type'] == 'web_app':
            keyboard.append([InlineKeyboardButton(btn['text'], web_app={"url": btn['url']})])
...
def post_manager_channel_keyboard():
    ...
    if channels:
        for ch in channels:
            ...
            ch_id = ch['channel_id']
            keyboard.append([
                InlineKeyboardButton(f"📢 {ch_name}", callback_data=f"pm_view_{ch_id}"),
...
def save_pinned_buttons(channel_id, buttons):
    try:
        pinned_buttons_col.update_one(
            {'channel_id': channel_id},
            {'$set': {'channel_id': channel_id, 'buttons': buttons, 'updated_at': datetime.utcnow()}},
            upsert=True
        )
        return True
    except Exception as e:
...
def pinned_manager_keyboard():
    ...
    if channels:
        for ch in channels:
            ...
            btns = ch.get('buttons', [])
            keyboard.append([
                InlineKeyboardButton(f"📌 Channel {ch_id} ({len(btns)} btn)", callback_data=f"pin_manage_{ch_id}"),
...
async def handle_post_manager_channel_post(message, context):
    ...
    # Save pending post
    code = save_pending_post(channel_id, message_id, media_type, file_id, caption, text)
    if not code:
        return
    ...
    info_text = (
        ...
        f"📁 Type: `{media_type}`\n"
        f"🔑 Code: `{code}`\n\n"
        f"নিচের বাটন থেকে action নিন:"
    )

    action_keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("➕ Button যোগ করুন", callback_data=f"pm_addbtn_{code}")],
        [InlineKeyboardButton("✏️ Caption/Text Edit করুন", callback_data=f"pm_editcap_{code}")],
        [InlineKeyboardButton("✅ যেমন আছে Post করুন", callback_data=f"pm_postasis_{code}")],
        [InlineKeyboardButton("🗑️ Delete করুন", callback_data=f"pm_delete_{code}")]
    ])

    try:
        # Forward the original post
        await context.bot.forward_message(
            chat_id=ADMIN_ID,
            from_chat_id=channel_id,
            message_id=message_id
        ...
        logger.info(f"✅ Post Manager: forwarded post {message_id} from {channel_id} to admin")
    except Exception as e:
...
async def show_pm_button_panel(query, code):
    """Show current buttons + add/remove/post options"""
    post = get_pending_post(code)
    if not post:
    ...
    for i, btn in enumerate(buttons):
        ...
        keyboard.append([
            InlineKeyboardButton(f"{icon} {btn['text']}", callback_data="noop"),
            InlineKeyboardButton("❌ Remove", callback_data=f"pm_rmbtn_{code}_{i}")
        ])

    keyboard.append([InlineKeyboardButton("➕ Button যোগ করুন", callback_data=f"pm_addbtn_{code}")])
    keyboard.append([InlineKeyboardButton("✅ এই buttons সহ Post এ Apply করুন", callback_data=f"pm_apply_{code}")])
    keyboard.append([InlineKeyboardButton("🗑️ Post Delete করুন", callback_data=f"pm_delete_{code}")])

    if buttons:
        text += "👇 বর্তমান buttons:"
    else:
...
async def handle_pm_callbacks(data, query, context, user_id):
    ...
    # --- APPLY BUTTONS TO POST ---
        ...
        reply_markup = build_post_buttons_keyboard(buttons, code) if buttons else None
    ...
    # --- DELETE POST ---
        ...
        try:
            ...
            await query.edit_message_text(f"❌ Delete করা যায়নি!\n\n`{str(e)[:200]}`", parse_mode=ParseMode.MARKDOWN)
    ...
    # --- REMOVE CHANNEL ---
        ...
        text = (
            "📢 **Post Manager**\n\n"
            "এই channel গুলো থেকে post হলে Admin কে forward করবে।\n"
            "Admin button যোগ করতে বা edit করতে পারবে।"
        )
        await query.edit_message_text(text, reply_markup=post_manager_channel_keyboard(), parse_mode=ParseMode.MARKDOWN)
...
async def handle_pinned_callbacks(data, query, context, user_id):
    ...
    if data == "pin_add_channel":
        ...
        text = (
            ...
            f"এই buttons টা pinned message এ থাকবে।\n"
            f"যতটা খুশি button দিতে পারবেন (url বা mini app):"
        )
...
async def handle_voice_upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # If already ogg/voice, send directly
            if file_ext == 'ogg' and message.voice:
                ...
                if result.returncode != 0:
                    # ffmpeg failed - try sending as-is
                    logger.warning(f"ffmpeg conversion failed, trying direct send")
                    with open(input_path, 'rb') as vf:
                        sent = await context.bot.send_voice(
                            chat_id=channel_id,
                            voice=vf
        ...
        logger.info(f"✅ Voice message sent to channel {channel_id}")
...
def main():
    ...
    application.add_handler(MessageHandler(
        (filters.AUDIO | filters.VOICE | filters.Document.AUDIO) & filters.User(ADMIN_ID),
        admin_message_handler
    ))
